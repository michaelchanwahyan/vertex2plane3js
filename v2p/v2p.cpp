...
std::vector<int> argSortVertexCoord(const std::vector<std::vector<float>>& vertex, const int axisIdx) {
    // axisIdx: == 0 -> using x axis to sort
    //          == 1 -> using y axis to sort
    //          == 2 -> using z axis to sort
    size_t Len = vertex.size();
    std::vector<int> idx(Len, 0);
    for (int i = 0; i < Len; i++) {
        idx[i] = i;
    }
    //std::sort(idx.begin(), idx.end(), [&vertex](int i1, int i2){return vertex[i1] < vertex[i2];});
    if (axisIdx == 0) {
        std::sort(idx.begin(), idx.end(), [&vertex](int i1, int i2) {return vertex[i1][0] < vertex[i2][0]; });
    }
    if (axisIdx == 1) {
        std::sort(idx.begin(), idx.end(), [&vertex](int i1, int i2) {return vertex[i1][1] < vertex[i2][1]; });
    }
    if (axisIdx == 2) {
        std::sort(idx.begin(), idx.end(), [&vertex](int i1, int i2) {return vertex[i1][2] < vertex[i2][2]; });
    }
    return idx;
}
...
int main()
{
    ...
    for (int wallIdx = 0; wallIdx < 5; wallIdx++) {
        ...
        std::vector<int> flat_plane_vertex_sort_order;
        ...
        {
            // detect the points for which the points are most negative in y-axis value
            flat_plane_vertex_sort_order = argSortVertexCoord(vertex_xyz, 1);
            // take the length of line joint by the two points
            // with most negative y coordinates as the width
            _x_diff = vertex_xyz[flat_plane_vertex_sort_order[0]][0] - vertex_xyz[flat_plane_vertex_sort_order[1]][0];
            _y_diff = vertex_xyz[flat_plane_vertex_sort_order[0]][1] - vertex_xyz[flat_plane_vertex_sort_order[1]][1];
            _3js_wall_width = std::sqrt(_x_diff * _x_diff + _y_diff * _y_diff);
            // take the length of line joint by the 2nd and 3rd points
            // with most negative y coordinates as the height
            _x_diff = vertex_xyz[flat_plane_vertex_sort_order[1]][0] - vertex_xyz[flat_plane_vertex_sort_order[2]][0];
            _y_diff = vertex_xyz[flat_plane_vertex_sort_order[1]][1] - vertex_xyz[flat_plane_vertex_sort_order[2]][1];
        ...
        // ---------------------------
        // rotation angle
        float _js_rotateX = 0.0f, _js_rotateY = 0.0f, _js_rotateZ = 0.0f;
        if (u[1] * u[1] > 0.001) {
            _js_rotateX = std::atanf(u[2] / u[1]);
        }
        else {
            _js_rotateX = u[2] > 0 ? 3.141592654f / 2.0f : -3.141592654f / 2.0f;
        }
        if (_z_diff > 10) {
            ...
            _x_diff = vertex_xyz[flat_plane_vertex_sort_order[1]][0] - vertex_xyz[flat_plane_vertex_sort_order[0]][0];
            _y_diff = vertex_xyz[flat_plane_vertex_sort_order[1]][1] - vertex_xyz[flat_plane_vertex_sort_order[0]][1];
            if (_x_diff * _x_diff > 0.001) {
                _js_rotateZ = std::atanf(_y_diff / _x_diff);
            }
            else {
                _js_rotateZ = _y_diff > 0 ? 3.141592654f / 2.0f : -3.141592654f / 2.0f;
            }
        }
